...
class Reflector:
    ...
    def _get_log_reflection_prompt(self) -> str:
        ...
        return (
            ...
            "Cover in order:\n"
            "1. Was the directional call correct? (cite the alpha figure, or the raw return when alpha is unavailable)\n"
    ...
    def reflect_on_final_decision(
        ...
        Intraday (4H) outcomes pass ``alpha_return=None`` — no benchmark has
        bars aligned with 24/7 crypto 4H windows, so the reflection judges the
        call by the raw return alone.
        """
        if alpha_return is None:
            alpha_line = (
                "Alpha: not computed (no benchmark aligns with this intraday"
                " timeframe) — judge the call by the raw return"
            )
        else:
            alpha_line = f"Alpha vs {benchmark_name}: {alpha_return:+.1%}"
        messages = [
